...
inline
bool operator==(const PetDTO &lhs, const PetDTO &rhs) {
    // TODO std::tie is not working with oatpp::Vector

    auto result = lhs.id == rhs.id && lhs.category == rhs.category &&
                  lhs.name == rhs.name &&
                  lhs.photoUrls == rhs.photoUrls && lhs.tags == rhs.tags &&
