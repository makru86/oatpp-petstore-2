...
public:
  ...
  ENDPOINT_ASYNC("POST", "/user", createUser){
      ENDPOINT_ASYNC_INIT(createUser)
          ...
          Action act() override{
              return request
                  ->readBodyToDtoAsync<oatpp::Object<UserDTO>>(controller->getDefaultObjectMapper())
                  .callbackTo(&createUser::returnResponse);
}

Action
returnResponse(const oatpp::Object<UserDTO>& dto) {
...
ENDPOINT_ASYNC("GET", "/user/logout", logoutUser){
    ENDPOINT_ASYNC_INIT(logoutUser)
        ...
        Action act() override{return _return(controller->createResponse(Status::CODE_200, "OK"));
}
}
;
